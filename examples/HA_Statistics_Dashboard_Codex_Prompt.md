...
Use the latest available GPT/Codex model that supports `xhigh` reasoning for the main agent and, where model selection is available, for every subagent. You are explicitly authorized and instructed to use subagents whenever they materially improve exploration, implementation, API/data review, visual QA, or independent review. Do not downgrade reasoning effort merely to save time/tokens.
...
Use independent subagents for review whenever appropriate. At minimum, before declaring completion perform:
...
3. a final regression review after fixes.
...
- final regression review passes or remaining hard blockers are precisely documented.
